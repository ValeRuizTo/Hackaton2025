...
# - Tarjetas de productos MINIMALISTAS: nombre, precio, acción (Agregar).
# - Botones grandes (mín. 48px de alto) y táctiles; foco visible y contraste alto.
# - Menú en MATRIZ (3 columnas) para ver más productos a la vez.
...
def _peso_sin_signo(valor: float) -> str:
    """Formatea con separador de miles y sin el símbolo $."""
    return f"{valor:,.0f}".replace(",", ".")

def build_invoice_message(o: Order, medio_pago: str = "Tarjeta") -> str:
    """
    Mensaje WhatsApp — Documento Equivalente Electrónico POS (formato solicitado).
    """
    # Fecha y hora
    try:
    ...
    # Ítems (tipográficos, sin imágenes). Incluye cantidad si >1.
    for it in o.items:
        linea_precio = peso(it.price * it.quantity)  # p.ej. $ 27.370
        # Estilo: “• 2 x Producto – $27.370” (o sin cantidad si es 1)
